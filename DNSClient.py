...
real_name_server = '8.8.8.8'  # Example: Google's Public DNS server
...
if __name__ == '__main__':
    ...
    print("Results match" if result else "Results don't match")

    # Print the IP address obtained from querying the local DNS server for nyu.edu
    result = query_local_dns_server('nyu.edu.', question_type)
    print("IP address obtained from querying local DNS server:", result)

    # Print the exfiltrated data obtained from the local DNS server for nyu.edu
    result = exfiltrate_info('nyu.edu.', question_type)
    print("Exfiltrated data from local DNS server:", result)
